...
class HuxiuAdapter(BaseAdapter):
    name = "huxiu"
    platform = "虎嗅"

    def __init__(self, config: Optional[AdapterConfig] = None):
        super().__init__(config)
        self.api_url = "https://www.huxiu.com/v2-action/article/tops"
    ...
    def parse(self, raw_data: list[dict]) -> list:
        ...
        for item in raw_data:
            try:
                ...
                item_id = item.get("aid", "")
                url = f"https://www.huxiu.com/article/{item_id}.html" if item_id else ""
                summary = item.get("summary", "")
                hot_score = item.get("count_info", {}).get("praise", 0)
                ...
                self.logger.warning(f"Error parsing huxiu item: {e}")
        return items
