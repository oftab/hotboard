...
class DoubanGroupAdapter(BaseAdapter):
    name = "douban_group"
    platform = "豆瓣小组"

    def __init__(self, config: Optional[AdapterConfig] = None):
        super().__init__(config)
        self.api_url = "https://www.douban.com/group/explore"
            ...
            try:
                ...
                if title and len(title) > 2:
                    items.append(self._create_hot_item(
                        title=title,
                        url=url,
                        summary=f"豆瓣小组热帖 - {title}",
                        hot_score=float(100 - i),
                        tags=["豆瓣", "小组"],
                    ))
            except Exception as e:
                self.logger.warning(f"Error parsing douban_group item: {e}")
        return items
