...
class HostlocAdapter(BaseAdapter):
    name = "hostloc"
    platform = "全球主机交流"

    def __init__(self, config: Optional[AdapterConfig] = None):
        super().__init__(config)
        self.api_url = "https://hostloc.com/"
            ...
            try:
                ...
                if title:
                    items.append(self._create_hot_item(
                        title=title,
                        url=f"https://hostloc.com/thread-{tid}-1-1.html",
                        summary=f"HostLoc热帖 - {title}",
                        hot_score=float(100 - i),
                        tags=["HostLoc", "主机"],
                    ))
            except Exception as e:
                self.logger.warning(f"Error parsing hostloc item: {e}")
        return items
