...
class IfengAdapter(BaseAdapter):
    name = "ifeng"
    platform = "凤凰网"

    def __init__(self, config: Optional[AdapterConfig] = None):
        super().__init__(config)
        self.api_url = "https://shankapi.ifeng.com/feedflow/shared/real_time_list/recommend/30"
    ...
    def parse(self, raw_data: list[dict]) -> list:
        ...
        for item in raw_data:
            try:
                ...
                self.logger.warning(f"Error parsing ifeng item: {e}")
        return items
